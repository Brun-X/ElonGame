	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		auto reset_game = [=](int colorState)
		{
			for(int x = 0; x <= FoVwidth; x++)
			{
				...
				{
					case 0 :	// red
					red = (colorState + 1) * x * 4; // 0 -255
					break;

					case 1 :
					red = colorState < 7 ? (4 - (colorState % 4)) * (8 + (x / 2))  : 0;
					green = (((colorState % 4) + 1) * x * 3);
					blue = (((colorState % 4) + 1) * x * 4) * 0.6f;
					break;

					case 2 :
					blue = colorState < 11 ? ((3 - (colorState % 4)) * (16 - x)) / 2 : 0;
					green = (((colorState % 4) + 1) * x * 4);
					break;

					case 3 :
					green = colorState < 15 ? (3 - (colorState % 4)) * (16 - x) : 0;
					blue = (((colorState % 4) + 1) * x * 4);
					break;
